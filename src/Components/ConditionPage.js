...
export default function ConditionPage() {
    ...
    function deleteTask(item) {
        axios.post(BASE_URL + 'command/sendData/', {'data': item, 'type': 'remove_condition', 'node_id': item['actuator']})
            .then(response => updateList())
            .catch(error => console.log(error.response.status));
    }

    function updateList() {
        axios.get(BASE_URL + 'tasks/cond/')
    ...
    function handle_click() {
        ...
        if (is_data_valid())
            axios.post(BASE_URL + 'command/sendData/', data)
                .then(response => {
